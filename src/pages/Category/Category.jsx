...
export default function CategoryPage() {
  ...
  useEffect(() => {
    ...
    if (subcategory) {
      ...
      async function getFilnAllNewProductes() {
        const data = await axios
          .get(`${baseUrl}/product/${state.main}/${state.sub}/find-new-product`)
          .then((res) => res.data.data);
        setSections([...data]);
      }
      ...
      async function getFilnAllNewProductes() {
        const data = await axios
          .get(`${baseUrl}/product/${state.main}/find-new-product`)
          .then((res) => res.data.data);
        setSections([...data]);
      }
  ...
  useEffect(() => {}, [
    category,
    subcategory,
    mainCategoryInfoValues,
    subCategoryInfoValues,
    products,
    sections,
  ]);
        ...
        <div css={styles.productCarousel}>
          {sections &&
            sections.map((item) => (
              <div
                key={item.productId}
                css={styles.productItem}
                onClick={() => navigate(`/product/${item.productId}`)}
              >
                <div
                  css={styles.itemGridImage}
                  style={{
                    backgroundImage: `url(${item.productThumbnailImageUrl})`,
                  }}
                />
